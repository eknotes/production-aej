...
@section('content')
    ...
    @php
        $statTotal = $totalMachines ?? \App\Models\Machine::count();
        $statActive = $activeMachines ?? \App\Models\Machine::where('status', 'active')->count();
        $statInactive = $inactiveMachines ?? \App\Models\Machine::where('status', 'inactive')->count();
    @endphp
    ...
    <div class="w-full font-sans text-slate-600 dark:text-slate-300">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Data Mesin</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Manajemen data mesin, peralatan, dan mapping
                    reject.</p>
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3 mb-8">
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Mesin</p>
            ...
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Mesin Aktif</p>
            ...
            <div class="flex flex-col xl:flex-row justify-between gap-4 items-center">
                <div class="flex flex-col sm:flex-row gap-3 w-full xl:w-auto flex-grow items-center">
                    <div class="relative w-full sm:max-w-xs group">
                        ...
                        {{-- PERBAIKAN: Gunakan class !pl-12 untuk padding kiri yang lebih besar --}}
                ...
                <div class="flex items-center gap-3 w-full xl:w-auto justify-end">
                    ...
                    <div class="relative">
                        ...
                        <div id="exportMenu"
                            ...
                            <div class="py-1">
                                <a href="{{ route('machines.export', 'excel') }}" target="_blank"
                                ...
                                <a href="{{ route('machines.export', 'pdf') }}" target="_blank"
        ...
        <div id="table-data"
                    ...
                    <thead class="bg-slate-50/80 dark:bg-slate-700/50">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nama
                                Mesin</th>
                            {{-- Field dari Modal Create Ditampilkan --}}
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Tipe
                                Reject</th>
                            <th
                    ...
                    <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-100 dark:divide-slate-700">
                        @forelse ($machines as $machine)
                            <tr class="table-row-hover transition-colors duration-150 align-top">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-slate-500 font-medium">
                                    {{ ($machines->currentPage() - 1) * $machines->perPage() + $loop->iteration }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    @if ($machine->rejectItems->isEmpty())
                                        <span class="text-slate-400 text-xs italic">- Kosong -</span>
                                    @else
                                        <div class="flex flex-wrap gap-1">
                                            @foreach ($machine->rejectItems as $reject)
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-100">
                                                    {{ $reject->name }}
                                                </span>
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="flex flex-col items-center justify-center gap-1.5">
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" class="sr-only peer status-toggle"
                                                data-id="{{ $machine->id }}"
                                                {{ $machine->status == 'active' ? 'checked' : '' }}>
                                            <div
                                                class="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-600">
                                            </div>
                                        </label>
                                        <span
                                            class="badge-status {{ $machine->status == 'active' ? 'text-emerald-600 bg-emerald-50' : 'text-slate-500 bg-slate-100' }}">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-20 text-center text-slate-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span class="text-sm font-medium">Data mesin tidak ditemukan.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">{{ $machines->links() }}</div>
        </div>
    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Tambah Mesin Baru</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Isi informasi mesin dan konfigurasi reject.</p>
                    </div>
                ...
                <div class="px-6 py-6 custom-scroll">
                    <form action="{{ route('machines.store') }}" method="POST" id="createForm">
                        @csrf
                        <div class="space-y-5">
                            <div class="relative">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Nama
                                    Mesin <span class="text-rose-500">*</span></label>
                            ...
                            <div class="relative">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Tipe
                                    Reject <span class="text-slate-400 font-normal text-xs">(Opsional)</span></label>
                                <select name="reject_items[]" class="select2-multi block w-full" multiple="multiple"
                                    data-placeholder="Cari & Pilih Jenis Reject...">
                                    @foreach ($rejectItems as $reject)
                                        <option value="{{ $reject->id }}">{{ $reject->name }}</option>
                                    @endforeach
                                </select>
                                <p class="text-[10px] text-slate-400 mt-1.5">Pilih jenis reject yang mungkin terjadi di
                                    mesin ini.</p>
                            </div>
                        </div>
                    </form>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Edit Data Mesin</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi mesin.</p>
                    </div>
                    ...
                    <form id="editForm" method="POST">
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="relative">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Tipe
                                    Reject <span class="text-slate-400 font-normal text-xs">(Opsional)</span></label>
                                <select id="edit_reject_items" name="reject_items[]" class="select2-multi block w-full"
                                    multiple="multiple" data-placeholder="Cari & Pilih Jenis Reject...">
                                    @foreach ($rejectItems as $reject)
                                        <option value="{{ $reject->id }}">{{ $reject->name }}</option>
                                    @endforeach
                                </select>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Detail Mesin</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Informasi lengkap mesin (Read-only).</p>
                    </div>
                ...
                <div class="px-6 py-6 custom-scroll">
                    <div class="space-y-5">
                        ...
                        <div class="relative">
                            <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Tipe
                                Reject Terdaftar</label>
                            <input type="text" id="view_reject_list" class="saas-input" readonly
                                placeholder="Tidak ada reject item">
                        </div>
                    </div>
                </div>
                <div class="bg-slate-50 dark:bg-slate-700/50 px-6 py-4 border-t border-slate-100 flex justify-end gap-3">
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div class="text-center">
                    ...
                    <h3 class="text-xl font-bold text-slate-800 dark:text-white">Import Data Mesin</h3>
                ...
                <form action="{{ route('machines.import') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="mt-4 text-right">
                        <a href="{{ route('machines.template') }}" target="_blank"
    ...
    <script>
        ...
        function openEditModal(machine) {
            ...
            $('#edit_status').val(machine.status).trigger('change');
            let selectedRejectIds = machine.reject_items.map(item => item.id);
            $('#edit_reject_items').val(selectedRejectIds).trigger('change');
            document.getElementById('editForm').action = '/machines/' + machine.id;
            openModal('editModal');
        }

        // VIEW MODAL FILL (Diubah menjadi Form Readonly)
        function openViewModal(machine) {
            document.getElementById('view_name').value = machine.name;
            let statusText = machine.status === 'active' ? 'Aktif' : 'Nonaktif';
            document.getElementById('view_status_text').value = statusText;
            let rejectNames = machine.reject_items.length > 0 ?
                machine.reject_items.map(item => item.name).join(', ') :
                '- Tidak ada reject item -';
            document.getElementById('view_reject_list').value = rejectNames;
            openModal('viewModal');
        }
        ...
        $(document).ready(function() {
            ...
            // Init Select2 Multi with data-placeholder support (PERBAIKAN)
            $('.select2-multi').each(function() {
                $(this).select2({
                    placeholder: $(this).data('placeholder') || 'Pilih...',
                    width: '100%',
                    dropdownParent: $(this).closest('.relative') // Keep logic safe
                });
            });

            // Re-bind Select2 parent when modals open to fix z-index issues (IMPORTANT)
            ...
            $('#createModal .select2-multi').select2({
                dropdownParent: $('#createModal'),
                width: '100%',
                placeholder: $('#createModal .select2-multi').data('placeholder')
            });
            ...
            $('#editModal .select2-multi').select2({
                dropdownParent: $('#editModal'),
                width: '100%',
                placeholder: $('#editModal .select2-multi').data('placeholder')
            });

            // AJAX Search
            let timeout = null;
            let xhr = null;
            window.fetchMachines = function(url = "{{ route('machines.index') }}") {
