...
@section('content')
    ...
    @php
        $totalDowntimes = \App\Models\Downtime::count();
        $activeDowntimes = \App\Models\Downtime::where('status', 'active')->count();
        $inactiveDowntimes = \App\Models\Downtime::where('status', 'inactive')->count();
    @endphp
    ...
    <div class="w-full font-sans text-slate-600 dark:text-slate-300">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Data Downtime</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Master data jenis kendala atau pemberhentian
                    mesin.</p>
            </div>
            <div class="flex gap-3">
                <button onclick="openModal('createModal')"
                    ...
                    <i class="fas fa-plus mr-2"></i> Tambah Downtime
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3 mb-8">
            ...
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Downtime</p>
                    <h3 class="text-2xl font-extrabold text-slate-800 dark:text-white">{{ $totalDowntimes }}</h3>
                </div>
                <div
            ...
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Downtime Aktif</p>
                    <h3 class="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">{{ $activeDowntimes }}</h3>
                </div>
                <div
            ...
            <div
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-extrabold text-slate-600 dark:text-slate-400">{{ $inactiveDowntimes }}</h3>
                </div>
                <div
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full xl:w-auto flex-grow items-center">
                    <div class="relative w-full sm:max-w-xs group">
                        ...
                        <input type="text" id="searchInput" value="{{ request('search') }}" class="saas-input !pl-12"
                            placeholder="Cari nama downtime...">
                    </div>
                ...
                <div class="flex items-center gap-3 w-full xl:w-auto justify-end">
                    ...
                    <div class="relative">
                        ...
                        <div id="exportMenu"
                            ...
                            <div class="py-1">
                                <a href="{{ route('downtimes.export', 'excel') }}" target="_blank"
                                ...
                                <a href="{{ route('downtimes.export', 'pdf') }}" target="_blank"
        ...
        <div id="table-data"
                        ...
                        @forelse ($downtimes as $downtime)
                            <tr class="table-row-hover transition-colors duration-150 align-top">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-slate-500 font-medium">
                                    {{ ($downtimes->currentPage() - 1) * $downtimes->perPage() + $loop->iteration }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-slate-800 dark:text-white">{{ $downtime->name }}
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-20 text-center text-slate-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span class="text-sm font-medium">Data downtime tidak ditemukan.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">{{ $downtimes->links() }}</div>
        </div>
    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Tambah Downtime Baru</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Isi informasi downtime baru.</p>
                    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Edit Downtime</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi downtime.</p>
                    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Detail Downtime</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Informasi lengkap downtime (Read-only).</p>
                    </div>
    ...
    <div id="importModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <form action="{{ route('downtimes.import') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="mt-4 text-right">
                        <a href="{{ route('downtimes.template') }}" target="_blank"
    ...
    <script>
        ...
        // EDIT MODAL FILL
        function openEditModal(downtime) {
            document.getElementById('edit_name').value = downtime.name;
            $('#edit_status').val(downtime.status).trigger('change');
            document.getElementById('editForm').action = '/downtimes/' + downtime.id;
            openModal('editModal');
        }

        // VIEW MODAL FILL (Form Readonly)
        function openViewModal(downtime) {
            document.getElementById('view_name').value = downtime.name;
            let statusText = downtime.status === 'active' ? 'Aktif' : 'Nonaktif';
        ...
        $(document).ready(function() {
            ...
            let timeout = null;
            let xhr = null;
            window.fetchDowntimes = function(url = "{{ route('downtimes.index') }}") {
            ...
            $(document).on('change', '.status-toggle', function() {
                ...
                $.ajax({
                    url: "/downtimes/" + id + "/toggle-status",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status
