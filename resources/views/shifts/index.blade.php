...
@section('content')
    ...
    @php
        ...
        $activeShifts = \App\Models\Shift::where('status', 'active')->count();
        $inactiveShifts = \App\Models\Shift::where('status', 'inactive')->count();
    @endphp

    {{-- CUSTOM CSS (SaaS Style & Dark Mode) --}}
    <style>
        /* Base Input Styles */
        .saas-input {
            height: 44px;
    ...
    <div class="w-full font-sans text-slate-600 dark:text-slate-300">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Shift Kerja</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Master data pengaturan waktu shift kerja.</p>
            </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3 mb-8">
            {{-- Total --}}
            <div
                class="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-subtle border border-slate-200 dark:border-slate-700 flex items-center justify-between group hover:border-brand-200 dark:hover:border-brand-500/50 transition-colors">
                <div>
                    <p class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Total
                        Shift</p>
                    <h3 class="text-2xl font-extrabold text-slate-800 dark:text-white">{{ $totalShifts }}</h3>
                </div>
                <div
            ...
            {{-- Active --}}
            <div
                class="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-subtle border border-slate-200 dark:border-slate-700 flex items-center justify-between group hover:border-emerald-200 dark:hover:border-emerald-500/50 transition-colors">
                <div>
                    <p class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Shift
                        Aktif</p>
                    <h3 class="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">{{ $activeShifts }}</h3>
                </div>
                <div
            ...
            {{-- Inactive --}}
            <div
                class="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-subtle border border-slate-200 dark:border-slate-700 flex items-center justify-between group hover:border-slate-300 dark:hover:border-slate-600 transition-colors">
                <div>
                    <p class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Nonaktif
                    </p>
                    <h3 class="text-2xl font-extrabold text-slate-600 dark:text-slate-400">{{ $inactiveShifts }}</h3>
                </div>
                <div
                ...
                <div class="flex items-center gap-3 w-full xl:w-auto justify-end">
                    ...
                    <div class="relative">
                        ...
                        <div id="exportMenu"
                            ...
                            <div class="py-1">
                                <a href="{{ route('shifts.export', 'excel') }}" target="_blank"
                                    class="block px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-emerald-600 transition-colors">
                                    <i class="fas fa-file-excel mr-2 text-emerald-500"></i>Download Excel
                                </a>
                                <a href="{{ route('shifts.export', 'pdf') }}" target="_blank"
                                    class="block px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-rose-600 transition-colors">
                                    <i class="fas fa-file-pdf mr-2 text-rose-500"></i>Download PDF
                                </a>
                            </div>
                        </div>
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-700">
                    ...
                    <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-100 dark:divide-slate-700">
                            ...
                            <tr class="table-row-hover transition-colors duration-150 align-top">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <div class="text-sm font-bold text-slate-800 dark:text-white">{{ $shift->name }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex flex-col items-center justify-center gap-1.5">
                                        ...
                                        <span
                                            class="badge-status {{ $shift->status == 'active' ? 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 dark:text-emerald-400' : 'text-slate-500 bg-slate-100 dark:bg-slate-700 dark:text-slate-400' }}"
                                            id="status-label-{{ $shift->id }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center items-center gap-1">
                                        <button onclick="openViewModal({{ $shift }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-20 text-center text-slate-500 dark:text-slate-400">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span class="text-sm font-medium">Data shift tidak ditemukan.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Tambah Shift Baru</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Isi informasi shift di bawah ini.</p>
                    </div>
                ...
                <div class="px-6 py-6 custom-scroll">
                    <form action="{{ route('shifts.store') }}" method="POST" id="createForm">
                        @csrf
                        <div class="space-y-5">
                            <div class="relative">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="name" required class="saas-input with-icon"
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Edit Data Shift</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Perbarui informasi shift.</p>
                    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Detail Shift</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Informasi lengkap shift (Read-only).
                        </p>
                    </div>
                ...
                <div
                    class="bg-slate-50 dark:bg-slate-700/50 px-6 py-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3">
                    <button onclick="closeModal('viewModal')"
                        class="px-5 py-2.5 bg-white border border-slate-300 dark:border-slate-600 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 transition-all">Tutup</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="importModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <form action="{{ route('shifts.import') }}" method="POST" enctype="multipart/form-data" class="mt-8"
                    id="importForm">
                    @csrf
                    <div
                    ...
                    <div class="mt-4 text-right">
                        <a href="{{ route('shifts.template') }}" target="_blank"
    ...
    <script>
        ...
        function openEditModal(shift) {
            ...
            $('#edit_status').val(shift.status).trigger('change');
            document.getElementById('editForm').action = '/shifts/' + shift.id;
            openModal('editModal');
        }

        // VIEW MODAL FILL (Form Readonly)
        function openViewModal(shift) {
            document.getElementById('view_name').value = shift.name;
            let statusText = shift.status === 'active' ? 'Aktif' : 'Nonaktif';
        ...
        $(document).ready(function() {
            ...
            let timeout = null;
            let xhr = null;
            window.fetchShifts = function(url = "{{ route('shifts.index') }}") {
