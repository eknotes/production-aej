...
@section('content')
    ...
    @php
        ...
        $activeOperators = \App\Models\Operator::where('status', 'active')->count();
        $inactiveOperators = \App\Models\Operator::where('status', 'inactive')->count();
    @endphp
    ...
    <div class="w-full font-sans text-slate-600 dark:text-slate-300">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Operator Produksi</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Dashboard manajemen data karyawan operator.</p>
            </div>
            <div>
                <button onclick="openModal('createModal')"
                    ...
                    <i class="fas fa-plus mr-2"></i> Operator Baru
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3 mb-8">
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Operator</p>
                    <h3 class="text-2xl font-extrabold text-slate-800 dark:text-white">{{ $totalOperators }}</h3>
                </div>
                <div
            ...
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Operator Aktif</p>
                    <h3 class="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">{{ $activeOperators }}</h3>
                </div>
                <div
            ...
            <div
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-extrabold text-slate-600 dark:text-slate-400">{{ $inactiveOperators }}</h3>
                </div>
                <div
                ...
                <div class="flex items-center gap-3 w-full xl:w-auto justify-end">
                    ...
                    <div class="relative">
                        ...
                        <div id="exportMenu"
                            ...
                            <div class="py-1">
                                <a href="{{ route('operators.export', 'excel') }}" target="_blank"
                                ...
                                <a href="{{ route('operators.export', 'pdf') }}" target="_blank"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-700">
                    <thead class="bg-slate-50/80 dark:bg-slate-700/50">
                        <tr>
                            <th
                                class="px-6 py-4 text-center text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-10">
                                #</th>
                            <th
                                class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider min-w-[200px]">
                                Nama Operator</th>
                            <th
                        ...
                        @forelse ($operators as $operator)
                            <tr class="table-row-hover transition-colors duration-150 align-top">
                                <td
                                    ...
                                    {{ ($operators->currentPage() - 1) * $operators->perPage() + $loop->iteration }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <div
                                                class="h-10 w-10 rounded-full bg-gradient-to-tr from-brand-100 to-brand-200 dark:from-brand-900/30 dark:to-brand-800/30 flex items-center justify-center text-brand-700 dark:text-brand-400 font-bold text-sm border border-brand-200 dark:border-brand-700">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex flex-col items-center justify-center gap-1.5">
                                        ...
                                        <span
                                            class="badge-status {{ $operator->status == 'active' ? 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 dark:text-emerald-400' : 'text-slate-500 bg-slate-100 dark:bg-slate-700 dark:text-slate-400' }}"
                                            id="status-label-{{ $operator->id }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center items-center gap-1">
                                        <button onclick="openViewModal({{ $operator }})"
                                        ...
                                        <button onclick="openEditModal({{ $operator }})"
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-20 text-center text-slate-500 dark:text-slate-400">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span class="text-sm font-medium">Data operator tidak ditemukan.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Tambah Operator Baru</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Isi informasi operator di bawah ini.
                        </p>
                    </div>
                ...
                <div class="px-6 py-6 custom-scroll">
                    <form action="{{ route('operators.store') }}" method="POST" id="createForm">
                        @csrf
                        <div class="space-y-5">
                            <div class="relative">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Nama
                                    Operator <span class="text-rose-500">*</span></label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Edit Data Operator</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Perbarui informasi operator.</p>
                    </div>
                            ...
                            <div class="relative">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="edit_name" name="name" required
                                        class="saas-input with-icon" placeholder="Nama operator">
                                </div>
                            </div>
                            <div class="relative">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Status
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Detail Operator</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Informasi lengkap operator
                            (Read-only).</p>
                    </div>
                ...
                <div
                    class="bg-slate-50 dark:bg-slate-700/50 px-6 py-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3">
                    <button onclick="closeModal('viewModal')"
                        class="px-5 py-2.5 bg-white border border-slate-300 dark:border-slate-600 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 transition-all sm:text-sm">Tutup</button>
                </div>
            </div>
        </div>
    </div>
            ...
            <div
                ...
                <form action="{{ route('operators.import') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="mt-4 text-right">
                        <a href="{{ route('operators.template') }}" target="_blank"
    ...
    <script>
        ...
        function openEditModal(operator) {
            ...
            $('#edit_status').val(operator.status).trigger('change');
            document.getElementById('editForm').action = '/operators/' + operator.id;
            openModal('editModal');
        }

        // VIEW MODAL FILL (Form Readonly)
        function openViewModal(operator) {
            document.getElementById('view_name').value = operator.name;
            let statusText = operator.status === 'active' ? 'Aktif' : 'Nonaktif';
        ...
        $(document).ready(function() {
            ...
            let timeout = null;
            let xhr = null;
            window.fetchOperators = function(url = "{{ route('operators.index') }}") {
