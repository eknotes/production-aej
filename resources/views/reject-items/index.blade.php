...
@section('title', 'Master Data Reject Items')
...
@section('content')
    ...
    @php
        $totalItems = \App\Models\RejectItem::count();
        $activeItems = \App\Models\RejectItem::where('status', 'active')->count();
        $inactiveItems = \App\Models\RejectItem::where('status', 'inactive')->count();
    @endphp
    ...
    <div class="w-full font-sans text-slate-600 dark:text-slate-300">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Reject Items</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Master data jenis cacat atau kerusakan produk.
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3 mb-8">
            ...
            <div
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-extrabold text-slate-800 dark:text-white">{{ $totalItems }}</h3>
                </div>
                <div
            ...
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Reject Aktif</p>
                    <h3 class="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">{{ $activeItems }}</h3>
                </div>
                <div
            ...
            <div
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-extrabold text-slate-600 dark:text-slate-400">{{ $inactiveItems }}</h3>
                </div>
                <div
                ...
                <div class="flex items-center gap-3 w-full xl:w-auto justify-end">
                    ...
                    <div class="relative">
                        ...
                        <div id="exportMenu"
                            ...
                            <div class="py-1">
                                <a href="{{ route('reject-items.export', 'excel') }}" target="_blank"
                                ...
                                <a href="{{ route('reject-items.export', 'pdf') }}" target="_blank"
        ...
        <div id="table-data"
                    ...
                    <thead class="bg-slate-50/80 dark:bg-slate-700/50">
                        <tr>
                            <th
                                ...
                                #</th>
                            <th
                                class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider min-w-[200px]">
                                Nama Item</th>
                        ...
                        @forelse ($rejectItems as $item)
                            <tr class="table-row-hover transition-colors duration-150 align-top">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-slate-500 font-medium">
                                    {{ ($rejectItems->currentPage() - 1) * $rejectItems->perPage() + $loop->iteration }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-slate-800 dark:text-white">{{ $item->name }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-600 border border-slate-200 dark:bg-slate-700/50 dark:text-slate-300 dark:border-slate-600">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex flex-col items-center justify-center gap-1.5">
                                        ...
                                        <span
                                            class="badge-status {{ $item->status == 'active' ? 'text-emerald-600 bg-emerald-50' : 'text-slate-500 bg-slate-100' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center items-center gap-1">
                                        ...
                                        <form action="{{ route('reject-items.destroy', $item->id) }}" method="POST"
                                            class="inline-block">
                                            @csrf @method('DELETE')
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-20 text-center text-slate-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span class="text-sm font-medium">Data item reject tidak ditemukan.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">{{ $rejectItems->links() }}
            </div>
        </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Tambah Reject Item</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Isi informasi item reject baru.</p>
                    </div>
                ...
                <div class="px-6 py-6 custom-scroll">
                    <form action="{{ route('reject-items.store') }}" method="POST" id="createForm">
                        @csrf
                        <div class="space-y-5">
                            <div class="relative">
                                ...
                                <input type="text" name="name" required class="saas-input"
                                    placeholder="Masukkan item reject">
                            </div>
                            <div class="relative">
                                <label
                                    class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Kategori
                                    <span class="text-red-500">*</span></label>
                                <select name="category_id" class="saas-input select2-modal" required
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Edit Reject Item</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi item reject.</p>
                    </div>
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="relative">
                                <label
                                    class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Kategori
                                    <span class="text-red-500">*</span></label>
                                <select id="edit_category_id" name="category_id" class="saas-input select2-modal"
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Detail Reject Item</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Informasi lengkap item (Read-only).</p>
                    </div>
                ...
                <div class="px-6 py-6 custom-scroll">
                    <div class="space-y-5">
                        ...
                        <div class="relative">
                            <label
                                class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Kategori</label>
                            <input type="text" id="view_category" class="saas-input" readonly>
                        </div>
                        <div class="relative">
                            <label
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div class="text-center">
                    ...
                    <h3 class="text-xl font-bold text-slate-800 dark:text-white">Import Data</h3>
                    <p class="text-sm text-slate-500 mt-2">Upload file .xlsx sesuai template.</p>
                </div>
                <form action="{{ route('reject-items.import') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="mt-4 text-right">
                        <a href="{{ route('reject-items.template') }}" target="_blank"
    ...
    <script>
        ...
        function openEditModal(item) {
            ...
            document.getElementById('editForm').action = '/reject-items/' + item.id;
            openModal('editModal');
        }
        ...
        function openViewModal(item) {
            ...
            document.getElementById('view_category').value = item.category ? item.category.name : '-';
            let statusText = item.status === 'active' ? 'Aktif' : 'Nonaktif';
        ...
        $(document).ready(function() {
            ...
            let timeout = null;
            let xhr = null;
            window.fetchRejectItems = function(url = "{{ route('reject-items.index') }}") {
            ...
            $(document).on('change', '.status-toggle', function() {
                ...
                $.ajax({
                    url: "/reject-items/" + id + "/toggle-status",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status
