...
@section('title', 'Master Data Koordinator')
...
@section('content')
    ...
    @php
        $totalCoordinators = \App\Models\Coordinator::count();
        $activeCoordinators = \App\Models\Coordinator::where('status', 'active')->count();
        $inactiveCoordinators = \App\Models\Coordinator::where('status', 'inactive')->count();
    @endphp
    ...
    <div class="w-full font-sans text-slate-600 dark:text-slate-300">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Koordinator Produksi</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Dashboard manajemen data koordinator lapangan.</p>
            </div>
            <div>
                <button onclick="openModal('createModal')"
                    ...
                    <i class="fas fa-plus mr-2"></i> Tambah Koordinator
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3 mb-8">
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Koordinator</p>
                    <h3 class="text-2xl font-extrabold text-slate-800 dark:text-white">{{ $totalCoordinators }}</h3>
                </div>
                <div
            ...
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Koordinator Aktif</p>
                    <h3 class="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">{{ $activeCoordinators }}
                    </h3>
                </div>
                <div
            ...
            <div
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-extrabold text-slate-600 dark:text-slate-400">{{ $inactiveCoordinators }}</h3>
                </div>
                <div
                ...
                <div class="flex items-center gap-3 w-full xl:w-auto justify-end">
                    ...
                    <div class="relative">
                        ...
                        <div id="exportMenu"
                            ...
                            <div class="py-1">
                                <a href="{{ route('coordinators.export', 'excel') }}" target="_blank"
                                ...
                                <a href="{{ route('coordinators.export', 'pdf') }}" target="_blank"
        ...
        <div id="table-data"
                        ...
                        @forelse ($coordinators as $coordinator)
                            <tr class="table-row-hover transition-colors duration-150 align-top">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-slate-500 font-medium">
                                    {{ ($coordinators->currentPage() - 1) * $coordinators->perPage() + $loop->iteration }}
                                </td>
                                    ...
                                    <div class="flex flex-col items-center justify-center gap-1.5">
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" class="sr-only peer status-toggle"
                                                ...
                                                {{ $coordinator->status == 'active' ? 'checked' : '' }}>
                                            <div
                                                class="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand-600">
                                            </div>
                                        </label>
                                        <span
                                            class="badge-status {{ $coordinator->status == 'active' ? 'text-emerald-600 bg-emerald-50' : 'text-slate-500 bg-slate-100' }}">
                                            {{ $coordinator->status == 'active' ? 'Aktif' : 'Nonaktif' }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center items-center gap-1">
                                        <button onclick="openViewModal({{ $coordinator }})"
                                            class="p-2 text-slate-400 hover:text-brand-600 hover:bg-brand-50 rounded-lg"
                                            title="Lihat Detail"><i class="fas fa-eye"></i></button>
                                        <button onclick="openEditModal({{ $coordinator }})"
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-20 text-center text-slate-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span class="text-sm font-medium">Data koordinator tidak ditemukan.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">{{ $coordinators->links() }}</div>
        </div>
    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Tambah Koordinator Baru</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Isi informasi koordinator di bawah ini.</p>
                    </div>
                ...
                <div class="px-6 py-6 custom-scroll">
                    <form action="{{ route('coordinators.store') }}" method="POST" id="createForm">
                        @csrf
                        <div class="space-y-5">
                            <div class="relative">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Nama
                                    Koordinator <span class="text-rose-500">*</span></label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Edit Data Koordinator</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi koordinator.</p>
                    </div>
                            ...
                            <div class="relative">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="edit_name" name="name" required
                                        class="saas-input with-icon" placeholder="Nama koordinator">
                                </div>
                            </div>
                            <div class="relative">
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Status
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div
                    class="bg-white dark:bg-slate-800 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Detail Koordinator</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Informasi lengkap koordinator (Read-only).</p>
                    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div
                ...
                <div class="text-center">
                    ...
                    <p class="text-sm text-slate-500 mt-2">Upload file .xlsx sesuai template untuk import data massal.</p>
                </div>
                <form action="{{ route('coordinators.import') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="mt-4 text-right">
                        <a href="{{ route('coordinators.template') }}" target="_blank"
    ...
    <script>
        ...
        function openEditModal(coordinator) {
            document.getElementById('edit_name').value = coordinator.name;
            $('#edit_status').val(coordinator.status).trigger('change');
            document.getElementById('editForm').action = '/coordinators/' + coordinator.id;
            openModal('editModal');
        }

        // VIEW MODAL FILL (Form Readonly)
        function openViewModal(coordinator) {
            document.getElementById('view_name').value = coordinator.name;
            let statusText = coordinator.status === 'active' ? 'Aktif' : 'Nonaktif';
        ...
        $(document).ready(function() {
            ...
            let timeout = null;
            let xhr = null;
            window.fetchCoordinators = function(url = "{{ route('coordinators.index') }}") {
            ...
            $(document).on('change', '.status-toggle', function() {
                ...
                $.ajax({
                    url: "/coordinators/" + id + "/toggle-status",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        status: status
